		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/purchase_information/table','add') || $check_action('/purchase_information/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/purchase_information/table','del') || $check_action('/purchase_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','store_name')" min-width="200">
					...
					<el-table-column prop="merchant_information" @sort-change="$sortChange" label="商户信息"
				v-if="user_group == '管理员' || $check_field('get','merchant_information')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_merchant_information(scope.row['merchant_information']) }}
				</template>
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','trade_name')" min-width="200">
					...
					<el-table-column prop="commodity_price" @sort-change="$sortChange" label="商品价格"
				v-if="user_group == '管理员' || $check_field('get','commodity_price')" min-width="200">
					</el-table-column>
					<el-table-column prop="resident_information" @sort-change="$sortChange" label="住户信息"
				v-if="user_group == '管理员' || $check_field('get','resident_information')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_resident_information(scope.row['resident_information']) }}
				</template>
					</el-table-column>
					<el-table-column prop="resident_name" @sort-change="$sortChange" label="住户姓名"
				v-if="user_group == '管理员' || $check_field('get','resident_name')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','purchase_quantity')" min-width="200">
					...
					<el-table-column prop="total_purchase_price" @sort-change="$sortChange" label="购买总价"
				v-if="user_group == '管理员' || $check_field('get','total_purchase_price')" min-width="200">
					</el-table-column>
					<el-table-column prop="shipping_address" @sort-change="$sortChange" label="配送地址"
				v-if="user_group == '管理员' || $check_field('get','shipping_address')" min-width="200">
					</el-table-column>
					<el-table-column prop="purchase_remarks" @sort-change="$sortChange" label="购买备注"
				v-if="user_group == '管理员' || $check_field('get','purchase_remarks')" min-width="200">
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','order_status')" min-width="200">
					...
					<el-table-column prop="delivery_information" @sort-change="$sortChange" label="配送信息"
				v-if="user_group == '管理员' || $check_field('get','delivery_information')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/purchase_information/table','set') || $check_action('/purchase_information/view','set') || $check_action('/purchase_information/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/purchase_information/table','set') || $check_action('/purchase_information/view','set') || $check_action('/purchase_information/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/purchase_information/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/purchase_information/get_list?like=0",
				url_del: "~/api/purchase_information/del?",
		...
		methods: {
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						let sqlwhere = "(";
															if(user_group=="商户用户"){
						sqlwhere+= "`merchant_information` = " + this.user.user_id + " or ";
					}
																							if(user_group=="住户用户"){
						sqlwhere+= "`resident_information` = " + this.user.user_id + " or ";
					}
			...
			get_user_resident_information(id){
				...
				return ret;
																					...
																					openPayModal(v){
				this.pay_obj.id = v.purchase_information_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/purchase_information/set?purchase_information_id="+this.pay_obj.id);
