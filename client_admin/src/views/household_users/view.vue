		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="住户姓名" prop="resident_name">
												<el-input id="resident_name" v-model="form_sub['resident_name']" placeholder="请输入住户姓名"
							  v-if="user_group === '管理员' || (form_sub['household_users_id'] && $check_field('set','resident_name')) || (!form_sub['household_users_id'] && $check_field('add','resident_name'))"></el-input>
					<div v-else-if="$check_field('get','resident_name')">{{form_sub['resident_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','household_gender') || $check_field('add','household_gender') || $check_field('set','household_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="住户性别" prop="household_gender">
								<el-select id="household_gender" v-model="form_sub['household_gender']"
						v-if="user_group === '管理员' || (form_sub['household_users_id'] && $check_field('set','household_gender')) || (!form_sub['household_users_id'] && $check_field('add','household_gender'))">
						<el-option v-for="o in list_household_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','household_gender')">{{form_sub['household_gender']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
			...
			return {
				...
				disabledObj:{
								"resident_name_isDisabled": false,
										"household_gender_isDisabled": false,
							},

				form_sub: {
								"resident_name":  '', // 住户姓名
										"household_gender":  '', // 住户性别
									    "user_id": 0,
					"household_users_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["household_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?household_users_id=" + form_sub["household_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/household_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/household_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/household_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/household_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/household_users/view','get');
