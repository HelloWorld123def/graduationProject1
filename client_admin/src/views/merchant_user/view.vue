		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="商户姓名" prop="merchant_name">
												<el-input id="merchant_name" v-model="form_sub['merchant_name']" placeholder="请输入商户姓名"
							  v-if="user_group === '管理员' || (form_sub['merchant_user_id'] && $check_field('set','merchant_name')) || (!form_sub['merchant_user_id'] && $check_field('add','merchant_name'))"></el-input>
					<div v-else-if="$check_field('get','merchant_name')">{{form_sub['merchant_name']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','merchant_gender') || $check_field('add','merchant_gender') || $check_field('set','merchant_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="商户性别" prop="merchant_gender">
								<el-select id="merchant_gender" v-model="form_sub['merchant_gender']"
						v-if="user_group === '管理员' || (form_sub['merchant_user_id'] && $check_field('set','merchant_gender')) || (!form_sub['merchant_user_id'] && $check_field('add','merchant_gender'))">
						<el-option v-for="o in list_merchant_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','merchant_gender')">{{form_sub['merchant_gender']}}</div>
							</el-form-item>
			</el-col>
				...
				<el-form-item label="店铺名称" prop="store_name">
												<el-input id="store_name" v-model="form_sub['store_name']" placeholder="请输入店铺名称"
							  v-if="user_group === '管理员' || (form_sub['merchant_user_id'] && $check_field('set','store_name')) || (!form_sub['merchant_user_id'] && $check_field('add','store_name'))"></el-input>
					<div v-else-if="$check_field('get','store_name')">{{form_sub['store_name']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
			...
			return {
				...
				disabledObj:{
								"merchant_name_isDisabled": false,
										"merchant_gender_isDisabled": false,
										"store_name_isDisabled": false,
							},

				form_sub: {
								"merchant_name":  '', // 商户姓名
										"merchant_gender":  '', // 商户性别
										"store_name":  '', // 店铺名称
									    "user_id": 0,
					"merchant_user_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["merchant_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?merchant_user_id=" + form_sub["merchant_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/merchant_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/merchant_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/merchant_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/merchant_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/merchant_user/view','get');
