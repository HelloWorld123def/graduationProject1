<template>
	<el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
		...
		<el-submenu index="notic"
			...
			<template slot="title"><i class="el-icon-warning-outline"></i><span>公告信息管理</span></template>
		...
		<el-submenu index="user"
			...
			<el-menu-item index="/user/table" v-show="user_group == '管理员'"><span>管理员</span></el-menu-item>
						<el-menu-item index="/household_users/table" v-show="user_group == '管理员' || $check_action('/household_users/table')"><span>住户用户</span></el-menu-item>
							<el-menu-item index="/merchant_user/table" v-show="user_group == '管理员' || $check_action('/merchant_user/table')"><span>商户用户</span></el-menu-item>
				...
				for (var i = 0; i < tables.length; i++) {
					...
					if (o.path.indexOf("/table") !== -1) {
						let list = ["exam","article", "article_type", ,"comment","forum","forum_type","notice"];
							list.push("household_users");
								list.push("merchant_user");
