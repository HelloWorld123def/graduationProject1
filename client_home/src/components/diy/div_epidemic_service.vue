<template>
	<div class="diy_details_box diy_div_epidemic_service">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/application_information/edit','add')" @click="to_form('/application_information/edit')" > 申请 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/epidemic_service/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/epidemic_service/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "epidemic_service_id",
					source_id: this.obj["epidemic_service_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/epidemic_service/set?epidemic_service_id=' + _this.obj["epidemic_service_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var epidemic_service_id = this.obj.epidemic_service_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "epidemic_service",
          source_field: "epidemic_service_id",
          source_id: epidemic_service_id
        }, (res) => {
