<template>
	<div class="diy_details page_epidemic_service" id="epidemic_service_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_epidemic_service'>
							<div_epidemic_service :obj="obj"></div_epidemic_service>
...
<script>
	import div_epidemic_service from "@/components/diy/div_epidemic_service.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/epidemic_service/set?epidemic_service_id=' + obj["epidemic_service_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.epidemic_service_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "epidemic_service",
					source_field: "epidemic_service_id",
					source_id: obj["epidemic_service_id"],
					user_id
				};
